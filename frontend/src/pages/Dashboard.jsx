    ...
    <div className="min-h-screen bg-bg grid-bg noise">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-20 overflow-hidden">
        ...
        >
          ...
          <motion.p variants={fade} className="text-text-2 text-lg max-w-2xl mx-auto mb-10">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {PROBLEM_BLOCKS.map((b, i) => (
              ...
              >
                ...
                <h3 className="font-display font-bold text-text mb-3">{b.title}</h3>
                <p className="text-text-2 text-sm leading-relaxed mb-6">{b.body}</p>
                <div className="flex items-end gap-2">
                  <span className="font-display font-extrabold text-3xl"
                        style={{ color: b.color }}>
                    {b.stat}
                  </span>
                  <span className="text-xs font-mono text-text-2 pb-1">{b.statLabel}</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <div key={tile.label} className="bg-bg-card rounded-xl p-4 glow-border">
                <div className="font-display font-bold text-2xl mb-1" style={{ color: tile.color }}>
                  {tile.val}
                </div>
                <div className="text-xs font-mono text-text-2">{tile.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-bg-card rounded-2xl glow-border overflow-hidden">
            <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
              <span className="font-display font-semibold text-text text-sm">High-Risk Districts</span>
              <span className="text-xs font-mono text-text-2">Sorted by flood coverage</span>
            </div>
                ...
                <thead>
                  <tr className="border-b border-white/5">
                    {['District','Flood %','Area km²','Pop. Exposed','Risk','Confidence'].map(h => (
                      <th key={h} className="text-left px-5 py-3 text-xs font-mono text-text-3 font-normal">
                        {h}
                      </th>
                    ))}
                  </tr>
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-xs" style={{ color: RISK_COLORS[d.risk]?.hex }}>
                            {d.floodPct}%
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-3"><RiskBadge risk={d.risk} size="xs" /></td>
                      <td className="px-5 py-3 font-mono text-xs text-text-2">
