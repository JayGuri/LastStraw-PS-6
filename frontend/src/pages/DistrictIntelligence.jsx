  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 border-b border-white/5 flex items-start justify-between"
        ...
        <div>
          <div className="text-xs font-mono text-text-3 mb-1">{district.id}</div>
          <h3 className="font-display font-bold text-xl text-text">{district.name}</h3>
          <div className="flex items-center gap-2 mt-2">
            <RiskBadge risk={district.risk} size="sm" />
            <span className="text-xs font-mono text-text-2">conf {(district.conf * 100).toFixed(0)}%</span>
          </div>
        </div>
        <button
          onClick={onClose}
          className="w-7 h-7 flex items-center justify-center rounded-lg
                     bg-white/5 hover:bg-white/10 text-text-2 hover:text-text transition-colors text-sm"
        >
          ✕
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={s.label} className="bg-bg rounded-xl p-3 border border-white/5">
              <div className="font-display font-bold text-lg" style={{ color: s.color }}>{s.val}</div>
              <div className="text-[10px] font-mono text-text-3">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        {/* Radar chart */}
        <div>
          <div className="text-xs font-mono text-text-2 mb-3 uppercase tracking-wider">
            Risk Profile
          </div>
        ...
        <div className="bg-bg rounded-xl p-3 border border-white/5">
          <div className="text-[10px] font-mono text-text-3 mb-2 uppercase tracking-wider">
            Data Quality
          </div>
...
export default function DistrictIntelligence() {
  ...
  return (
    <div className="min-h-screen bg-bg noise pt-16 flex flex-col">
      <div className="flex-1 flex flex-col max-w-[1600px] mx-auto w-full px-4 py-4 gap-4">
          ...
          <div className="flex items-center gap-6 text-xs font-mono">
            {[
              ...
              <div key={s.label} className="text-center">
                ...
                <div className="text-text-3">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          {/* Overlay toggle */}
          <div className="flex gap-1 bg-bg-2 rounded-xl p-1 border border-white/5">
            {OVERLAYS.map(o => (
              <button key={o.id}
          ...
          <div className="ml-auto text-xs font-mono text-text-3">
            {RUN_META.runId} · {RUN_META.sceneDate}
          </div>
        </div>

        {/* Main content: [hex map] [sidebar list] [detail panel] */}
        <div className="flex-1 grid grid-cols-12 gap-4" style={{ minHeight: 500 }}>

          {/* Hex map */}
          <div className="col-span-12 lg:col-span-6 xl:col-span-7 bg-bg-card rounded-2xl
                          glow-border overflow-hidden relative">
            <div className="absolute inset-0 p-4">
              <DistrictHexMap onSelect={(d) => setSelectedDistrict(d)} />
            </div>

            {/* Legend */}
            <div className="absolute bottom-3 left-3 bg-bg/80 backdrop-blur-sm
                            rounded-xl p-3 border border-white/5 space-y-1.5">
              ...
              {overlay === 'risk' ? (
                Object.entries(RISK_COLORS).map(([k, c]) => (
                  <div key={k} className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-text-2">{c.label}</span>
                  </div>
                ))
              ) : (
          ...
          <div className={`${selectedDistrict ? 'hidden xl:flex' : 'flex'}
            ...
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
              <span className="text-xs font-mono text-text-2">Districts</span>
              <select
                value={sidebarSort}
                onChange={e => setSidebarSort(e.target.value)}
                className="text-[10px] font-mono text-text-2 bg-transparent border-0 outline-none cursor-pointer"
              >
              ...
              {sorted.map(d => {
                ...
                return (
                  <button key={d.id}
                    ...
                    className={`w-full text-left px-4 py-3 border-b border-white/[0.03]
                                hover:bg-white/[0.02] transition-colors
                                ${isSelected ? 'bg-gold/5 border-l-2 border-l-gold' : ''}`}
                  >
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="text-xs font-medium text-text">{d.name}</span>
          ...
          <div className="col-span-12 lg:col-span-6 xl:col-span-3">
            <AnimatePresence mode="wait">
              {selectedDistrict ? (
                ...
                >
                  <div className="w-16 h-16 rounded-2xl bg-gold/10 border border-gold/15
                                  flex items-center justify-center text-3xl mb-5 animate-float">
                    🗺️
                  </div>
                  <div className="font-display font-bold text-text mb-2">
                    Select a District
                  </div>
                  ...
                  <div className="mt-6 grid grid-cols-2 gap-3 w-full text-left">
                    {[
                      ...
                      <div key={s.label} className="bg-bg rounded-xl p-3 border border-white/5">
                        <div className="font-mono font-bold text-gold-lt text-sm">{s.val}</div>
                        <div className="text-[10px] font-mono text-text-3">{s.label}</div>
                      </div>
                    ))}
                  </div>
